  ...
  ngOnInit(): void{
    ...
    if(this.stored){
      this.userInfoEdit = this.formBuilder.group({
        usernameEdit: [this.stored.username, Validators.compose([Validators.required, Validators.pattern(this.usernamePattern)])],
        firstNameEdit:  [this.stored.firstName, Validators.required],
        lastNameEdit:  [this.stored.lastName,Validators.required],
        ageEdit: [this.stored.age, Validators.compose([Validators.required, Validators.min(18), Validators.max(65)])],
        emailEdit: [this.stored.email, ([Validators.required, Validators.email])],
        passwordEdit: [this.stored.password, Validators.compose([Validators.required, Validators.pattern(this.passwordPattern)])],
        confirmPasswordEdit: [this.stored.confirmPassword, Validators.required],
        loggedOn: ['']
      },
      {
